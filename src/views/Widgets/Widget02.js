...
class Widget02 extends Component {
  render() {
    const { className, cssModule, header, mainText, icon, color, footer, link, children, variant, button, ...attributes } = this.props;
    ...
    const ButtonText = function() {
      if (button) {
        return (<div><Button className="text-muted text-uppercase font-weight-bold font-xs">{mainText}</Button></div>)
      }
      else {
        return(<div className="text-muted text-uppercase font-weight-bold font-xs">{mainText}</div>)
      }
    }
